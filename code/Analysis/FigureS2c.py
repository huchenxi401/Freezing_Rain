...
def read_nclimgrid_data_with_years(file_path, start_year, end_year):
    try:
        ...
        for idx, t in enumerate(time_values):
            ...
            if is_djf and is_in_range:
                if season_year not in yearly_data:
                    yearly_data[season_year] = []

                temp_data = ds['tavg'].values[idx, :, :]
                yearly_data[season_year].append(temp_data)
...
def main():
    ...
    years_array, annual_temps = calculate_annual_us_mean(yearly_data, us_mask)
    
    temps_smoothed = moving_average(annual_temps, window=5)
    
    output_file = './figure/nclimgrid_US_winter_temperature_timeseries_DJF.png'
