...
def read_nclimgrid_data(file_path, start_year, end_year, filter_month=None):
    try:
        ...
        temp_data = ds['tavg'].values[month_mask, :, :]
...
def calculate_Feb_percentile(temp_data):
    Feb = np.nanmean(temp_data, axis=0)
    return Feb
...
def main():
    ...
    if len(results) > 0:
        ...
        if len(results) == 2:
            ...
            title = 'Change in February Temperature\n(2011-2025) - (1996-2010)'
            output_file = './figure/nclimgrid_February_temp_change.png'
