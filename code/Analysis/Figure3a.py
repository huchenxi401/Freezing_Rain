...
def load_and_process_data(excel_file="../../data/freezing_rain_events_county_llm.xlsx"):
    ...
    over_12hr_events = df_clean[df_clean['DURATION_HOURS'] > 12]   
    return df_clean

def calculate_county_statistics_by_year_12hr(df, time_period=None, month=None):
    filter_desc = []
    df_filtered = df.copy()
    ...
    df_filtered = df_filtered[df_filtered['DURATION_HOURS'] > 12]
    filter_desc.append(">12hr")
...
def calculate_county_statistics_over_12hr(df, time_period=None, month=None):
    ...
    df_over_12hr = df_filtered[df_filtered['DURATION_HOURS'] > 12]

    county_stats = df_over_12hr.groupby(['STATE_ABBREV', 'COUNTY_CLEAN']).size().reset_index(name='EVENT_COUNT')

    county_stats['EVENT_COUNT'] = county_stats['EVENT_COUNT'] / 15.0
    
    return county_stats

def perform_ttest_by_county_12hr(yearly_data_dict, month=None):
...
def create_maps_diff(merged_gdf, states_gdf, counties_alaska, states_alaska, alaska_data, 
    ...
    if len(counties_with_events) > 0:
        counties_with_events.plot(column='EVENT_COUNT', 
                                 ...
                                 vmin=-0.5, 
                                 vmax=0.5,
                                 legend_kwds={'shrink': 0.8, 'label': 'Difference in February Events Yr⁻¹ (Duration>12hr)',
    ...
    if 'significant' in merged_gdf.columns:
        ...
        if len(significant_counties) > 0:
            ...
            light_sig = significant_counties_copy[
                (significant_counties_copy['EVENT_COUNT'] >= -0.37) & 
                (significant_counties_copy['EVENT_COUNT'] <= 0.37)
            ]
    ...
    plt.tight_layout()
    output_file = output_path / f'freezing_rain_over_12hr_count_map_diff{month_suffix}.png'
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
    plt.close()
   
    return {
        'over_12hr_count_diff': output_file
    }
    ...
    try:
        ...
        for month in months:
            ...
            for period in time_periods:
                yearly_stats = calculate_county_statistics_by_year_12hr(df, period, month)
                yearly_data_dict[period] = yearly_stats

            ttest_results = perform_ttest_by_county_12hr(yearly_data_dict, month)
            all_ttest_results[month] = ttest_results

            period1_stats = calculate_county_statistics_over_12hr(df[df['TIME_PERIOD'] == '1996-2010'], month=month)
            period2_stats = calculate_county_statistics_over_12hr(df[df['TIME_PERIOD'] == '2011-2025'], month=month)
            
            all_counties = set()
            for stats in [period1_stats, period2_stats]:
                for _, row in stats.iterrows():
                    all_counties.add((row['STATE_ABBREV'], row['COUNTY_CLEAN']))
