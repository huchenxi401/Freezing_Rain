...
def calculate_county_annual_trends(df):
    
    annual_counts = df.groupby(['STATE_ABBREV', 'COUNTY_CLEAN', 'BEGIN_YEAR']).agg({
        'EVENT_ID': 'count'
    }).reset_index()
    ...
    significant_counties = trend_df[trend_df['SIGNIFICANT']].shape[0]
    total_counties = len(trend_df)
...
def create_trend_map(merged_gdf, states_gdf, counties_alaska, states_alaska, alaska_data, output_dir="./figure"):
    ...
    if len(counties_with_data) > 0:
        counties_with_data.plot(column='TREND_SLOPE', 
                               ...
                               vmin=-0.08,  
                               vmax=0.08, 
                               legend_kwds={'shrink': 0.8, 'label': 'Freezing Rain Trend (Events Yr⁻²)',
    ...
    plt.tight_layout()
    plt.savefig(output_path / 'freezing_rain_annual_trend_map.png', dpi=300, bbox_inches='tight')
    plt.close()
    
    return {
        'annual_trend': output_path / 'freezing_rain_annual_trend_map.png'
    }
...
def main():
    ...
    try:
        ...
        trend_data = calculate_county_annual_trends(df)
        
        merged_gdf = merge_data_with_geography(trend_data, counties_gdf)
        
        alaska_merged = merge_alaska_data(trend_data, counties_alaska)
        
        map_files = create_trend_map(merged_gdf, states_gdf, counties_alaska, states_alaska, alaska_merged, output_dir)
