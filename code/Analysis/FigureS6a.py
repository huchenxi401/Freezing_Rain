...
def process_monthly_duration_data(df, target_month):
    monthly_df = df[df['BEGIN_MONTH'] == target_month].copy()

    if len(monthly_df) == 0:
        return None

    episode_avg_duration = monthly_df.groupby(['EPISODE_ID', 'BEGIN_YEAR'])['DURATION_HOURS'].mean().reset_index()
    episode_avg_duration.columns = ['EPISODE_ID', 'Year', 'Avg_Duration']

    episodes_over_12hr = episode_avg_duration[episode_avg_duration['Avg_Duration'] > 12]
 
    yearly_over_12hr_count = episodes_over_12hr.groupby('Year').size().reset_index(name='Count_Over_12hr')

    all_years = pd.DataFrame({'Year': range(1996, 2026)})
    yearly_over_12hr_count = all_years.merge(yearly_over_12hr_count, on='Year', how='left')
    yearly_over_12hr_count['Count_Over_12hr'] = yearly_over_12hr_count['Count_Over_12hr'].fillna(0)

    return yearly_over_12hr_count, episodes_over_12hr, episode_avg_duration

def create_monthly_duration_trend_plot(yearly_over_12hr_count, target_month, output_dir="./figure"):

    if yearly_over_12hr_count is None or len(yearly_over_12hr_count) < 3:
        return None

    x_vals = yearly_over_12hr_count['Year'].values
    y_vals = yearly_over_12hr_count['Count_Over_12hr'].values
    unique_years = x_vals
    ...
    if target_month==2:
        plt.ylabel('February Events Yr⁻² (duration>12hrs)', fontsize=12)
    plt.legend(fontsize=12)  
    plt.xticks(range(1996, 2026, 5))
    
    plt.tight_layout()
    
    output_file = f"{output_dir}/duration_over_12hr_trend_month{target_month:02d}.png"
    try:
...
def main():
    ...
    for target_month in target_months:
        result = process_monthly_duration_data(df, target_month)
        
        if result is None:
            continue
            
        yearly_over_12hr_count, episodes_over_12hr, episode_avg_duration = result
        output_file = create_monthly_duration_trend_plot(yearly_over_12hr_count, target_month, output_dir)
        if output_file:
