...
def read_era5_humidity_data(file_path):
    try:
        ...
        hum_data = ds['q'].values

        lon_grid, lat_grid = np.meshgrid(lons, lats)
        
        return hum_data, lat_grid, lon_grid, pressure_levels, time_values, ds
...
def calculate_county_centroid_humidity(hum_data, lats, lons, pressure_levels, 
    ...
    selected_level_indices = np.where(level_mask)[0]

    feb_mask = time_values.month == 2
    feb_years = time_values[feb_mask].year

    data_multilevel = hum_data[np.ix_(np.where(feb_mask)[0], selected_level_indices, [lat_idx], [lon_idx])]
    data_multilevel = data_multilevel.squeeze() 
    hum_mean = np.nanmean(data_multilevel, axis=1)*1000.0 
    climatology = np.nanmean(hum_mean)
    yearly_hum_diff = {}
    for year, diff in zip(feb_years, hum_mean):
        if year not in yearly_hum_diff:
            yearly_hum_diff[year] = []
        yearly_hum_diff[year].append(diff)
    
    for year in yearly_hum_diff:
        yearly_hum_diff[year] = np.nanmean(yearly_hum_diff[year])-climatology
    
    return yearly_hum_diff

def calculate_all_counties_humidity(hum_data, lats, lons, pressure_levels, 
    ...
    for idx, row in hotspot_counties.iterrows():
        ...
        yearly_hum_diff = calculate_county_centroid_humidity(
            hum_data, lats, lons, pressure_levels, time_values, row['geometry']
        )
        
        county_hum_data[county_key] = yearly_hum_diff
        
    return county_hum_data
...
def merge_humidity_and_events(county_hum_data, county_year_counts):

    data_list = []
    
    for county_key in county_hum_data:
        yearly_hums = county_hum_data[county_key]
        
        for year in range(1996, 2026):
            if year in yearly_hums:
                hum_diff = yearly_hums[year]
...
def plot_scatter_with_regression(merged_df, output_file):
    ...
    ax.set_xlabel('Low- Mid Level Humidity Anomaly [g/kg]', fontsize=10)
...
def main():
    ...
    hum_data, lats, lons, pressure_levels, time_values, ds = read_era5_humidity_data(era5_file)

    county_hum_data = calculate_all_counties_humidity(
        hum_data, lats, lons, pressure_levels, time_values, hotspot_counties
    )
    ...
    merged_df = merge_humidity_and_events(county_hum_data, county_year_counts)
