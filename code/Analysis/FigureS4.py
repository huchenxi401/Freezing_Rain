...
def analyze_monthly_trend(df, month, month_name):

    monthly_df = df[df['BEGIN_MONTH'] == month]
    yearly_episodes = monthly_df.groupby('BEGIN_YEAR')['EPISODE_ID'].nunique().reset_index()
    ...
    print(f"number trend: {slope:.3f} h/yr (95% CI: {ci_lower_weak:.3f} to {ci_upper_weak:.3f}, P={p_value_weak:.4f})")
    ...
    if month==12:
        plt.ylabel('December Events Yr⁻¹', fontsize=12)
    if month==1:
        plt.ylabel('January Events Yr⁻¹', fontsize=12)
    ...
    output_file = f'./figure/freezing_rain_{month_name.lower()}_trend.png'
    ...
    return {
        ...
        'yearly_data': yearly_episodes,
        'slope': model.coef_[0],
        'r_squared': model.score(x_vals.reshape(-1, 1), y_vals),
        'mean_events': y_vals.mean(),
        'std_events': y_vals.std()
    }
...
def main():
    ...
    df = df.dropna(subset=['BEGIN_MONTH'])
    ...
    for month, month_name in months_to_analyze:
        result = analyze_monthly_trend(df, month, month_name)
